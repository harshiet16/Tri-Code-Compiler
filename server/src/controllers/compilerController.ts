...
export const loadCode = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (existingCode.ownerInfo && String(existingCode.ownerInfo) !== String(userId)) {
      return res.status(403).send({ message: "You don't have permission to view this code!" });
    }
...
export const getAllCodes = async (req: AuthRequest, res: Response) => {
  try {
    const allCodes = await Code.find({ ownerInfo: req._id }).sort({
      createdAt: -1,
    });
